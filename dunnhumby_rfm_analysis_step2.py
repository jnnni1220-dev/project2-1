...
print("--- Dunnhumby RFM 분석: 고객 세그먼트별 분포 및 특성 분석 ---")
...
try:
    ...
    plt.title('Dunnhumby Customer Segment Distribution', fontsize=16)
    plt.xlabel('Customer Segment')
    plt.ylabel('Number of Customers')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, 'dunnhumby_segment_distribution.png'))
    plt.close()
    print(f"\n'{plots_dir}/dunnhumby_segment_distribution.png'에 고객 세그먼트 분포 차트를 저장했습니다.")
    ...
    print("\n\n--- Dunnhumby 고객 세그먼트별 평균 RFM 특성 ---")
    print(segment_characteristics)

    # Visualize RFM Characteristics
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(20, 6))
    fig.suptitle('Dunnhumby RFM Characteristics by Customer Segment', fontsize=16)
    ...
    plt.savefig(os.path.join(plots_dir, 'dunnhumby_segment_rfm_characteristics.png'))
    plt.close()
    print(f"\n'{plots_dir}/dunnhumby_segment_rfm_characteristics.png'에 세그먼트별 RFM 특성 차트를 저장했습니다.")
    
    print("\nDunnhumby 고객 세그먼트 분석 완료.")

except FileNotFoundError:
    print(f"오류: '{rfm_segments_path}' 파일을 찾을 수 없습니다. RFM 값 계산 및 세분화가 먼저 완료되어야 합니다.")
except Exception as e:
    print(f"Dunnhumby RFM 세그먼트 분석 중 오류가 발생했습니다: {e}")
