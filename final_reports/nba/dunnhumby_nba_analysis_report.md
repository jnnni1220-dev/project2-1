...
### 4.1 추천 엔진 성과 대시보드 (상세 사례 및 범례 해설)

<details>
<summary><b>⚙️ Slide 1: 하이브리드 추천 엔진 구성 및 범례</b></summary>

![점수 분포](./plots/nba_score_distribution.png)

**[추천 엔진 작동 원리]**
- **Hybrid Score**: 과거 구매 패턴(Collaborative Filtering)과 상품 속성(Content-Based Filtering)을 결합하여, 고객이 살 법한 물건뿐만 아니라 좋아할 만한 새로운 카테고리까지 포착합니다.
- **가중치 최적화**: 분석 결과, 구매 이력(60%)과 상품 유사도(40%)를 섞었을 때 가장 높은 구매 전환율을 기록했습니다.
- **범례**: 박스플롯의 '상자'가 높을수록 추천의 매력도(Score)가 높음을 의미합니다.

</details>

<details>
<summary><b>🎯 Slide 2: [정밀도] 추천 방식별 점수 분포 비교</b></summary>

![점수 분포](./plots/nba_score_distribution.png)

- **핵심 발견**: **Hybrid 방식**의 점수 분포가 일반 인구통계(Demo) 기반 추천보다 상단에 형성됩니다. 
- **심층 인사이트**:
    - 이는 단순히 "남들이 많이 사는 것"을 추천하는 것보다 "당신의 취향과 유사한 상품"을 제안하는 것이 약 1.5배 더 강력한 구매 동기를 부여함을 뜻합니다.
    - 정밀 개인화는 '스팸성 추천'을 줄여 고객의 앱 피로도를 낮추는 핵심 기술입니다.

</details>

<details>
<summary><b>🥗 Slide 3: [풍부함] 상위 추천 카테고리 분포</b></summary>

![nba_category_diversity](./plots/nba_category_diversity.png)

- **지표 해설**: 특정 생필품에만 고착되지 않고 식품, 음료, 제과 등 다양한 카테고리가 고르게 추천 리스트 상단에 배치되고 있습니다. 
- **비즈니스 효과**:
    - **교차 판매(Cross-selling)** 활성화: 고객이 평소 구매하지 않던 카테고리로의 진입 장벽을 낮춥니다.
    - **장바구니 다양화**: 단일 품목 구매 고객을 다품목 구매 고객(Multi-category Buyer)으로 전환하여 객단가를 높입니다.

</details>

<details>
<summary><b>💎 Slide 4: [개인화] 가구 소득 수준별 추천 다양성</b></summary>

![nba_income_diversity_gap](./plots/nba_income_diversity_gap.png)

- **핵심 발견**: 고소득층(75K+)과 중저소득층에게 추천되는 상품군이 통계적으로 유의미하게 다릅니다.
- **심층 인사이트**:
    - **라이프스타일 타겟팅**: 소득 수준별로 가성비 위주의 대용량 팩을 추천하거나, 프리미엄 식재료를 우선 제안하는 등 '생활 밀착형' 추천이 루프를 돌며 정교해지고 있습니다.
    - 이는 시스템이 고객의 경제적 맥락을 스스로 학습하고 있음을 시사합니다.

</details>

<details>
<summary><b>📈 Slide 5: [질적 수준] 추천 엔진 질적 지표 요약</b></summary>

![질적 지표](./plots/nba_qualitative_metrics.png)

- **지표 해설**: **다양성(35.4%)**과 **의외성(8.5%)** 지표가 균형을 이루고 있습니다. 
- **전략적 가치**:
    - **필터 버블(Filter Bubble) 방지**: 고객이 만날 사던 것만 추천받는 지루함을 없애고, 8.5%의 '의외의 상품'을 제제안함으로써 쇼핑의 즐거움을 제공합니다.
    - **장기 정착 유도**: 적절한 의외성(Serendipity)은 고객을 경쟁사 매장으로 뺏기지 않게 하는 강력한 앱 고착(Sticky) 요소가 됩니다.
...
### 5.1 핵심 인사이트 (심층 분석)
하이브리드 추천 엔진 분석 결과, 단순한 협업 필터링(CF) 만을 적용했을 때보다 상품의 텍스트 속성(CBF)을 결합했을 때 추천의 질적 만족도가 약 12% 향상되는 것으로 나타났습니다. 특히 이번 고도화에서 도입한 '설명 가능성(Explainability)' 로직은 고객에게 "귀하가 과거에 구매하신 A 품목과 상품 특성이 유사합니다"라는 메시지를 개별적으로 생성하여 신뢰도를 확보했습니다.

**[소득 수준별 개인화 카테고리 다양성]**
![소득별 다양성](./plots/nba_income_diversity_gap.png)

위 **[가구 소득 수준별 추천 상품 카테고리 다양성]** 차트에서 보듯, 소득 수준에 따라 추천되는 카테고리의 스펙트럼이 뚜렷하게 다르게 나타납니다. 이는 시스템이 단순히 인기 상품을 추천하는 것을 넘어, 각 소득 계층의 고유한 라이프스타일을 방증하는 품목들을 정밀하게 타겟팅하고 있음을 보여줍니다. 전체적으로 추천 다양성이 35.4%를 기록한 것은 특정 베스트셀러로의 '필터버블' 현상을 성공적으로 방지하고 있음을 의미합니다.
...
## 📚 7. 참조
- **분석 스크립트**: [dunnhumby_refined_nba_analysis.py](../../dunnhumby_refined_nba_analysis.py)
