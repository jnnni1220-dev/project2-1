...
summary_report_path = 'cross_selling_summary_report.md'
print(f"Generating Summary Report: {summary_report_path}...")

with open(summary_report_path, 'w', encoding='utf-8') as f:
    f.write("# 교차 판매 분석 최종 요약 보고서 (MBA Summary)\n\n")
    
    f.write("## 1. 개요 (Overview)\n")
    f.write(f"- **분석 규모**: 상위 50개 상품, 50만 건 샘플 데이터.\n")
    f.write(f"- **총 발견 규칙**: {len(rules)}개 (최소 신뢰도 0.1%, 향상도 1.01 이상).\n")
    f.write(f"- **데이터 안정성**: **{stability_score}** (교차 검증 중복 발견율).\n")
    f.write(f"- **최고 향상도(Max Lift)**: {rules['lift'].max():.2f} (일반적 기대치 대비 판매량 증가 배수).\n\n")

    f.write("## 2. 전략적 핵심 상품군 (Key Bundles to Watch)\n")
    f.write("### A. Top 3 파워 번들 (Volume-Driver)\n")
    f.write("> **특징**: 가장 많이 팔리면서도 함께 구매되는 필승 조합. 매장의 메인 진열 대상입니다.\n\n")
    f.write("| 상품 A | 상품 B | Lift | 전략 |\n")
    f.write("| :--- | :--- | :--- | :--- |\n")
    for _, row in power_bundles.head(3).iterrows():
        f.write(f"| {row['antecedents']} | {row['consequents']} | {row['lift']:.2f} | 동반 진열 (No 할인) |\n")
    f.write("\n")

    f.write("### B. Top 3 히든 잼 (Profit-Driver)\n")
    f.write("> **특징**: 아직 대중적이지 않지만, 아는 사람은 꼭 같이 사는 조합. 개인화 타겟팅 대상입니다.\n\n")
    f.write("| 상품 A | 상품 B | Lift | 전략 |\n")
    f.write("| :--- | :--- | :--- | :--- |\n")
    for _, row in hidden_gems.head(3).iterrows():
        f.write(f"| {row['antecedents']} | {row['consequents']} | {row['lift']:.2f} | 쿠폰 / 추천 발송 |\n")
    f.write("\n")

    f.write("## 3. 실행 가이드 (Action Plan)\n")
    f.write("- **즉시 실행**: 계산대 주변에 '파워 번들' 중 부피가 작은 상품(껌, 사탕 등)을 배치하십시오.\n")
    f.write("- **이번 주 과제**: '히든 잼' 조합을 활용하여 멤버십 고객 대상의 SMS/App 푸시 메시지 시나리오를 기획하십시오.\n")
    f.write("- **참고**: 상세한 분석 과정과 전체 리스트는 [**Detailed Report**](./cross_selling_opportunities.md)를 참조하십시오.\n")
